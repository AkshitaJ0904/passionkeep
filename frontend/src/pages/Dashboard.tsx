  ...
  return (
    <div data-passion={passion} style={{ minHeight: '100vh', position: 'relative' }}>
      ...
      <motion.nav initial={{ y: -80 }} animate={{ y: 0 }} style={{
        ...
        padding: '16px 30px',
        background: `rgba(10,10,26,${0.7 + scrollDepth * 0.2})`,
        backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--passion-border)', transition: 'background 0.3s'
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 24 }}>{passionInfo.emoji}</span>
          <div>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, color: 'var(--passion-accent)' }}>PassionKeep</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={logout} style={{ background: 'transparent', border: '1px solid var(--passion-border)', borderRadius: 8, padding: '6px 12px', color: 'var(--passion-muted)', cursor: 'pointer', fontSize: 12 }}>
            Logout
          </button>
        </div>
      </motion.nav>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'overview' && (
            <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', padding: '60px 40px 40px', position: 'relative' }}>
                <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} style={{ maxWidth: 700 }}>
                  <div style={{ fontSize: 14, color: 'var(--passion-accent)', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 16, fontWeight: 600 }}>Welcome back</div>
                  <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(36px, 6vw, 72px)', fontWeight: 900, lineHeight: 1.1, marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="btn-primary" onClick={() => setShowLogForm(true)} style={{ fontSize: 16, padding: '16px 40px' }}>
                      ✨ Log Today's Session
                    </motion.button>
                    <button className="btn-ghost" onClick={() => setActiveTab('insights')}>🧠 Get AI Insights</button>
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8 }}
                  ...
                  {[
                    ...
                    <motion.div key={i} whileHover={{ y: -5 }} className="glass-card" style={{ padding: 24 }}>
                      <div style={{ fontSize: 28, marginBottom: 8 }}>{stat.icon}</div>
                      <div style={{ fontSize: 32, fontWeight: 800, color: 'var(--passion-accent)', fontFamily: 'var(--font-display)' }}>{stat.value}{stat.suffix}</div>
                      <div style={{ fontSize: 12, color: 'var(--passion-muted)', textTransform: 'uppercase', letterSpacing: 1 }}>{stat.label}</div>
                    </motion.div>
                  ))}
                </motion.div>
              ...
              <div style={{ padding: '60px 40px', minHeight: '60vh' }}>
                ...
                {recentSessions.length === 0 ? (
                  <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="glass-card" style={{ padding: 60, textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'grid', gap: 20 }}>
                    {recentSessions.map((s, i) => (
                      <motion.div key={s._id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
              ...
              <div style={{ padding: '80px 40px', background: 'rgba(0,0,0,0.3)' }}>
                <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
                  ...
                  <div style={{ fontSize: 60, marginBottom: 20 }}>💡</div>
                  <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 36, marginBottom: 20 }}>Remember why you started</h2>
          ...
          {activeTab === 'log' && (
            <motion.div key="log" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ padding: '40px', maxWidth: 700, margin: '0 auto' }}>
              <SessionForm onClose={() => setActiveTab('overview')} />
            </motion.div>
          )}

          {activeTab === 'journal' && (
            <motion.div key="journal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ padding: '40px' }}>
              <JournalView />
            </motion.div>
          )}

          {activeTab === 'insights' && (
            <motion.div key="insights" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ padding: '40px', maxWidth: 800, margin: '0 auto' }}>
              <AIInsightPanel />
            </motion.div>
          )}

          {activeTab === 'stats' && (
            <motion.div key="stats" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ padding: '40px' }}>
              <StatsPanel />
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <AnimatePresence>
        {activeTab === 'overview' && (
          <motion.button initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} whileHover={{ scale: 1.1 }}
            onClick={() => setActiveTab('log')}
            style={{
              position: 'fixed', bottom: 70, right: 20, zIndex: 40,
              width: 56, height: 56, borderRadius: '50%',
              background: 'linear-gradient(135deg, var(--passion-accent), var(--passion-accent))',
      ...
      <AnimatePresence>
        {showLogForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
